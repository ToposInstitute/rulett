...
#[cfg(test)]
pub(crate) fn toy_model_chem() -> Model {
    let decls = toy_model_chem_decls();
    Model::parse(toy_signature_chem(), decls).unwrap()
}
...
#[cfg(test)]
fn toy_model_chem_decls() -> [ModelDecl; 9] {
    [
...
mod tests {
    ...
    fn parse() {
        ...
        "#]];
        expected.assert_eq(&toy_model_chem().to_string());
    }
}
